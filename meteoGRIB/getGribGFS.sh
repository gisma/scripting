...
if [[ "$1" == '-h' ]]; then
    ...
    echo "       ./getGribGFS.sh -d=YYYYMMDD -wl=DDD.DD -el=DDD.DD -nl=DDD.DD -sl=DDD.DD -st=0 -et=96 -nc=FALSE -g1=FALSE"
    echo""
    ...
    echo " -st=NN     :  start time [0] of forecast"
    echo " -et=NN     :  end time [96]  of forecast"
    ...
    echo " "
    echo " example   :  ./getGribGFS.sh -d=20150620 -wl=-10.5 -el=20.25 -nl=60.75 -sl=40.0"
    exit 0
fi
...
# remove single files
rm -r gfs.t${charStartTime}z.pgrb2.0p25.f???
